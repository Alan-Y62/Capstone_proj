...
//route for generating data
router.get('/generate', async (req,res) => {
    const re = await generateRepairs()
...
router.get('/repairh', async (req,res) => {
    const abc123 = await Repair.find()
    res.render('./admin/repairHA', {completed: abc123})
})
...
router.get('/repair_admin/d/:id/', async (req,res) => {
    await Repair.findByIdAndUpdate(req.params.id, {status: 'completed'})
    res.redirect('/m/repair_admin')
})
